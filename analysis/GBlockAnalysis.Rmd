...
```{r}
spikein_counts_df <- read_delim("~/Documents/GitHub/gblock-detection/spikeins_xii_results/stats/spikeins_data.txt") %>%
  group_by(SampleID) %>%
  # Remove unnecessary parts and extract Well and SDSI
  mutate(
    Well = str_extract(SampleID, "(^.*-[A-Z][0-9]+)-", group = 1),
    Well = str_replace(Well, "(SDSI1andSDSI2-|SDSI[0-9]-|negcontrol-|original-|SDSI[0-9]_[0-9]-|SDSI[0-9]_[0-9]_[0-9]-|SDSI[0-9],[0-9]-|SDSI[0-9],[0-9],[0-9]-)", ""),
    SDSI = str_extract(SampleID, "(SDSI[0-9]|original|negcontrol)"),
    SDSI = case_when(
      grepl("SDSI1andSDSI2", SampleID ) ~ "SDSI1_SDSI2",
      grepl("SDSI1,3&4", SampleID ) ~ "SDSI1_SDSI3_SDSI4",
      grepl("SDSI1_3_4", SampleID) ~  "SDSI1_SDSI3_SDSI4",
      TRUE ~ SDSI
    ),
  ...
  mutate(
    ...
    Spikeins = ifelse(!grepl("^SDSI", Spikeins), "None", Spikeins)
  )
...
amplicons_df <- read_delim("~/Documents/GitHub/gblock-detection/spikeins_xii_results/amplicon_coverage.txt") %>%
...
metadata_df <-  read_csv("~/Documents/GitHub/gblock-detection/Spike-In_METADATA_XII.csv")
dilutions_df <- metadata_df %>%
  select(
    SampleID = `Sample name for sequencing`,
    Density = `Parasite density of sample`,
    Spikeins = spikeins,
    Well
  ) %>%
  mutate(
    Density = replace_na(Density, 0),
    SIDilution = ifelse((Density > 0 & !grepl("original", SampleID) | grepl("template", SampleID)), "1:10", "0"),
    SIPrimerDilution = ifelse((Density > 0 & !grepl("original", SampleID) | grepl("template", SampleID)), "1:250", "0"),
    # Well = str_extract(SampleID, "(^[A-Z][0-9]+)-", group = 1),
    Well = Well,
    ...
    Spikeins = str_replace_all(Spikeins, "_", ","), # Convert underscores to commas for separation
    Spikeins = str_split(Spikeins, ",") # Split into individual Spikeins
  ) %>%
  unnest(Spikeins) %>%  # Create separate rows for each Spikein
  mutate(Spikeins = ifelse(!grepl("^SDSI", Spikeins), "None", Spikeins))
...
joined_df <- amplicons_df %>%
  ...
  left_join(dilutions_df %>% select(-SampleID), by = c("Well", "Spikeins"))
...
```{r}
alleledata_df <- read.table("mad4hatter-runs/spikeins_xii_mad4hatter_results/allele_data.txt", sep = "\t", header = TRUE) %>%
  mutate(
    Well = str_extract(SampleID, "(^.*-[A-Z][0-9]+)-", group = 1),
    Well = str_replace(Well, "(SDSI1andSDSI2-|SDSI[0-9]-|negcontrol-|original-|SDSI[0-9]_[0-9]-|SDSI[0-9]_[0-9]_[0-9]-|SDSI[0-9],[0-9]-|SDSI[0-9],[0-9],[0-9]-)", "")
  ) %>%
...
interaction_groups_df <- joined_df %>%
  distinct(Interaction, ControlType)

denisty_df <- dilutions_df %>%
  distinct(Well_exp = Well, Density) %>%
  mutate(Density = ifelse(Density >= 1000, 1000, 
                          ifelse(Density >= 100, 100, Density)))

final_results_df <- final_results_with_pseudocigar %>%
